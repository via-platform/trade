...
module.exports = class TradeView {
    ...
    bindOrderEvents(){
        ...
        this.disposables.add(this.order.onDidUpdateType(this.didUpdateType.bind(this)));
    ...
    didUpdateMarket(){
        ...
        if(this.order.market){
            //We don't necessarily need to do anything when the quote prices change (i.e. re-render the UI)
            //These are simply required for when the user wants to prefill prices
            this.ticker = this.order.market.ticker.subscribe(this.update.bind(this));
            this.quote = this.order.market.quotes.subscribe(() => {});

            //We don't necessarily know if the ticker is already initialized for this market, but it may be if the user is watching this somewhere else
            const last = this.order.market.ticker.last();

            if(last){
                if(via.config.get('trade.prefillLimitPrice') && ['limit', 'stop-limit'].includes(this.order.type) && !this.order.limit){
                    this.order.limit = last;
                }

                if(via.config.get('trade.prefillStopPrice') && ['stop-market', 'stop-limit'].includes(this.order.type) && !this.order.stop){
                    this.order.stop = last;
                }
            }
        }
        ...
        if(this.group){
            this.group.market = this.order.market;
        }
    }
